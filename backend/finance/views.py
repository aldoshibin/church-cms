...
class FundViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        """
        Aggregate 'This Month Collected' and 'Total Collected' for the
        set of Funds matching the current filters (fund_type, is_active,
        search) — same params the Fund list endpoint accepts.
        """
        qs = self.filter_queryset(self.get_queryset())
        fund_ids = list(qs.values_list('id', flat=True))

        now = timezone.now().date()
        month_start = now.replace(day=1)

        donations_qs = Donation.objects.filter(fund_id__in=fund_ids)

        this_month_total = donations_qs.filter(date__gte=month_start).aggregate(
            t=Sum('amount'))['t'] or 0
        all_time_total = donations_qs.aggregate(t=Sum('amount'))['t'] or 0
...
class DonationViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        """
        Aggregate 'This Month' and 'Total' donation amounts for whatever
        is currently filtered (search, payment_method, fund, date range —
        same params the Donation list endpoint accepts).
        """
        qs = self.filter_queryset(self.get_queryset())
        ...
        this_month_total = qs.filter(date__gte=month_start).aggregate(t=Sum('amount'))['t'] or 0
        all_time_total = qs.aggregate(t=Sum('amount'))['t'] or 0
...
class ExpenseViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        """
        Aggregate 'This Month' and 'Total' expense amounts for whatever
        is currently filtered (search, category, status, date range —
        same params the Expense list endpoint accepts).
        """
        qs = self.filter_queryset(self.get_queryset())
