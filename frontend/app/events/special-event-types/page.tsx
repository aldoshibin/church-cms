...
import { Plus, Edit2, Trash2, Tag, RefreshCw, Sparkles } from 'lucide-react';
import { toast } from '@/components/ui/Toast';
import ConfirmDialog from '@/components/ui/ConfirmDialog';

export default function SpecialEventTypesPage() {
  ...
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const { data } = await specialEventTypesApi.list();
      setTypes(data.results || data);
    } catch { toast.error('Failed to load special event types'); }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await specialEventTypesApi.delete(deleteId);
      toast.success('Special event type deleted');
      fetchTypes();
    } catch (err: any) {
    ...
    <DashboardLayout title="Special Event Types" subtitle="Manage Special Event types">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => router.push('/events/special-event-types/add')}
            ...
            {loading ? (
              <tr><td colSpan={5} className="text-center py-16">
                <div className="flex flex-col items-center gap-2 text-gray-400">
                  ...
                  <span className="text-sm">Loading special event types...</span>
                </div>
              </td></tr>
            ) : types.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-16">
                <div className="flex flex-col items-center gap-2 text-gray-400">
                  ...
                  <p className="font-medium text-gray-500">No special event types yet</p>
                  <p className="text-xs">Click "Add Event Type" to create your first one.</p>
                </div>
              </td></tr>
            ) : types.map((t: any) => (
              <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-5 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-purple-100 text-purple-600 rounded-xl flex items-center justify-center shrink-0">
                      <Sparkles size={15} />
                    </div>
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => router.push(`/events/special-event-types/${t.id}/edit`)} title="Edit"
      ...
      {deleteId && (
        <ConfirmDialog
          title="Delete Special Event Type"
