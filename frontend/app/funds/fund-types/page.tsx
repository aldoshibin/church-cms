...
import { Plus, Edit2, Trash2, Tag, RefreshCw, DollarSign, Search } from 'lucide-react';
...
export default function FundTypesPage() {
  ...
  const fetchTypes = async () => {
    ...
    try {
      ...
      const { data } = await fundTypesApi.list(params);
    ...
    } catch { toast.error('Failed to load fund types'); }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await fundTypesApi.delete(deleteId);
  ...
  return (
    <DashboardLayout title="Fund Types" subtitle="Manage giving fund categories">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Fund Types</h1>
          <p className="text-gray-500 text-sm mt-1">{count} type{count !== 1 ? 's' : ''} defined</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => router.push('/funds/fund-types/add')}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden overflow-x-auto">
        ...
        <table className="w-full">
          <thead style={{ background: '#f8fafc' }}>
            <tr>
              {['Type Name', 'Key', 'Funds Using This', 'Status', 'Actions'].map(h => (
            ...
            {loading ? (
              <tr><td colSpan={5} className="text-center py-16">
                <div className="flex flex-col items-center gap-2 text-gray-400">
                  ...
                  <span className="text-sm">Loading fund types...</span>
                </div>
              </td></tr>
            ) : types.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-16">
                <div className="flex flex-col items-center gap-2 text-gray-400">
                  ...
                  <p className="font-medium text-gray-500">{search || statusFilter ? 'No fund types match' : 'No fund types yet'}</p>
                  <p className="text-xs">{search || statusFilter ? 'Try adjusting filters.' : 'Click "Add Fund Type" to create your first one.'}</p>
                </div>
              </td></tr>
            ) : types.map((t: any) => (
              <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-5 py-3 text-sm text-gray-600">{t.fund_count} fund{t.fund_count !== 1 ? 's' : ''}</td>
                <td className="px-5 py-3">
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => router.push(`/funds/fund-types/${t.id}/edit`)} title="Edit"
      ...
      {deleteId && (
        <ConfirmDialog
          ...
          message="Existing funds using this type will keep showing the old type key. Continue?"
          confirmLabel="Delete"
