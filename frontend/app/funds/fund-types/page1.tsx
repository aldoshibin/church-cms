...
import { Plus, Edit2, Trash2, Tag, RefreshCw, DollarSign } from 'lucide-react';
...
export default function FundTypesPage() {
  ...
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const { data } = await fundTypesApi.list();
    ...
    <DashboardLayout title="Fund Types" subtitle="Manage giving fund categories">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{types.length} types defined</p>
        </div>
            ...
            {loading ? (
              ...
              <tr><td colSpan={5} className="text-center py-16">
                <div className="flex flex-col items-center gap-2 text-gray-400">
                  ...
                  <p className="font-medium text-gray-500">No fund types yet</p>
                  <p className="text-xs">Click "Add Fund Type" to create your first one.</p>
                </div>
              </td></tr>
            ) : types.map((t: any) => (
