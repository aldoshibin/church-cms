...
export default function FundsPage() {
  ...
  const [stats, setStats] = useState({ this_month_total: 0, all_time_total: 0, fund_count: 0 });
  ...
  const fetchStats = async () => {
    ...
    try {
      const { data } = await fundsApi.stats(buildFilterParams());
      setStats(data);
  ...
  useEffect(() => { fetchStats(); }, [search, fundType, statusFilter]);
  ...
  // Summary totals now come from fundsApi.stats() (see fetchStats above),
  // which correctly aggregates across ALL filtered funds server-side —
  // not just the current page. This resolves the earlier limitation.
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {statsLoading ? (
            ...
            <p className="text-3xl font-bold text-indigo-600">{stats.fund_count}</p>
          )}
          <p className="text-xs text-gray-400 mt-1">{search || fundType || statusFilter ? 'matching filters' : 'total funds'}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-sm text-gray-500 mb-1">This Month Collected</p>
          {statsLoading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {statsLoading ? (
            <div className="h-9 flex items-center"><div className="w-5 h-5 border-2 border-amber-200 border-t-amber-600 rounded-full animate-spin" /></div>
          ) : (
            <p className="text-3xl font-bold text-amber-600">₹{stats.all_time_total.toLocaleString('en-IN')}</p>
          )}
          <p className="text-xs text-gray-400 mt-1">{search || fundType || statusFilter ? 'for matching funds' : 'all time'}</p>
        </div>
      </div>
