...
const inp =
  'w-full bg-white text-gray-900 text-sm px-3 py-2.5 border border-gray-300 placeholder-gray-400 transition-all focus:outline-none focus:border-[#fff] focus:ring-2 focus:ring-[#3F51B5]/20';
// const inp = 'w-full bg-white text-gray-900 text-sm  px-3 py-2.5 outline-none transition-all border placeholder-gray-400 focus:border-[#3f51b5]  focus:ring-indigo-100';
// const inp =
  // 'w-full bg-white text-gray-900 text-sm px-3 py-2.5 outline-none transition-all border border-gray-300 placeholder-gray-400 focus:border-[#3F51B5]  focus:ring-[#3F51B5]/20';
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    else if (typeof window !== 'undefined' && localStorage.getItem('family_access_token'))
      ...
      <div className="w-full" style={{ maxWidth: 900 }}>
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden flex" style={{ minHeight: 520 }}>
          ...
          <div
            ...
            style={{ width: 450, background: '#111827' }}
          >
            <div>
              ...
              <ul className="space-y-3 " style={{paddingTop:"50px"}}>
                {(tab === 'admin' ? [
            ...
            <div className="text-xs" style={{ color: '#4b5563' }}>
              © 2026 Grace Church · All rights reserved
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center p-10">
            ...
            {/* Tab switcher */}
            {/* <div className="flex mb-8  overflow-hidden border border-gray-200 bg-gray-50">
...
<div className="flex mb-8 overflow-hidden border border-gray-200 bg-gray-50">
  {(['admin', 'family'] as Tab[]).map(t => (
    <button
      ...
      style={{
        backgroundColor: tab === t ? '#3F51B5' : 'transparent',
      }}
    >
      {t === 'admin' ? 'Admin / Staff' : 'Family'}
    </button>
  ))}
</div>
            {/* Admin form */}
            {tab === 'admin' && (
              <>
                <h2 className="text-2xl font-bold text-gray-900 mb-1 mt-5">Admin Login</h2>
                ...
                <form onSubmit={handleAdmin} className="space-y-4">
                  ...
                  <button type="submit" disabled={loading}
                    className="w-full py-3 text-sm font-bold text-white bg-primary hover:bg-indigo-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
                    {loading
                ...
                <div className="mt-5 flex items-center gap-2 bg-indigo-50 border-l-4 border-indigo-400 rounded-r-lg px-3 py-2.5 text-xs text-gray-600">
                  <span className="text-indigo-500 font-bold">Demo:</span>
            ...
            {tab === 'family' && (
              <>
                <h2 className="text-2xl font-bold text-gray-900 mb-1 mt-5">Family Portal</h2>
                ...
                <div className="mt-5 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2.5 text-xs text-amber-700">
                  Your credentials were emailed when your family was registered. Contact the church office for help.
                </div>
              </>
            )}
