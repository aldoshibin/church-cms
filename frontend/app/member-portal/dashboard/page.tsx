...
import {
  ...
  LogOut, Mail, Phone, MapPin, TrendingUp, Heart, Clock, Users, Wallet
} from 'lucide-react';
...
// Same palette the admin Sidebar.tsx uses, so the member portal visually
// matches the admin dashboard exactly: bg-gray-900 base, gray-800 active
// background, orange-500 left accent bar on the active item.
const FUND_COLORS = [
  'bg-indigo-100 text-indigo-700', 'bg-green-100 text-green-700', 'bg-amber-100 text-amber-700',
  'bg-pink-100 text-pink-700', 'bg-blue-100 text-blue-700', 'bg-purple-100 text-purple-700',
];
...
export default function MemberPortalDashboard() {
  ...
  if (!data) return null;
  const {
    member, family, family_members, stats, recent_donations, fund_totals,
    recent_attendance, upcoming_events, giving_trend
  } = data;

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Sidebar — matches admin Sidebar.tsx exactly: bg-gray-900, w-64,
          gray-800 active background + orange-500 left accent bar */}
      ...
      <div className="ml-64!" style={{marginLeft:"250px"}}>
        {/* Header */}
        <header className="sticky top-0 left-64 right-0 h-14 bg-white border-b border-gray-200 flex items-center justify-between px-6 z-20">
          <div>
        ...
        <main className="pt-[72px] p-6">
          ...
          {/* FUND PAID — totals grouped by fund */}
          {tab === 'funds' && (
            <>
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Fund Paid</h1>
              ...
              {fund_totals?.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {fund_totals.map((f: any, i: number) => {
                    ...
                    const colorClass = FUND_COLORS[i % FUND_COLORS.length];
                    return (
                      <div key={f.fund_id ?? 'general'} className="bg-white rounded-xl border border-gray-200 p-5">
                        <div className="flex items-center justify-between mb-3">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${colorClass}`}>{f.fund_name}</span>
                    ...
                    return (
                      <div key={ev.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
                        <div className="flex items-start gap-4">
                          {hasDays ? (
                            <div className="w-14 h-14 bg-indigo-50 rounded-xl flex flex-col items-center justify-center shrink-0 gap-0.5 p-1">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="space-y-1 text-sm text-gray-500">
                              {ev.time && <div className="flex items-center gap-1.5"><Clock size={13} />{ev.time}</div>}
