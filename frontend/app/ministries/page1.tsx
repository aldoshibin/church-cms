...
import { Plus, Edit2, Trash2, Search, Users, Calendar, Clock, RefreshCw, Church } from 'lucide-react';
    ...
    <DashboardLayout title="Ministries & Groups" subtitle="Manage church ministries and groups">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{ministries.length} ministries registered</p>
        </div>
          ...
          <thead style={{ background: '#f8fafc' }}>
            <tr>
              {['Ministry Name', 'Leader', 'Meeting Day', 'Meeting Time', 'Members', 'Status', 'Actions'].map(h => (
                <th key={h} className="text-left px-5 py-3 text-xs font-bold uppercase tracking-wider" style={{ color: '#455a64' }}>
                  {h}
                </th>
              ))}
            </tr>
              ...
              <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-indigo-100 text-primary rounded-xl flex items-center justify-center shrink-0">
                      <Church size={15} />
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-[#263238]">{m.name}</p>
                      {m.description && (
                ...
                <td className="px-5 py-3">
                  {m.leader_name ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-[#263238]">{m.leader_name}</span>
                    </div>
                  ) : (
                    <span className="text-sm text-[#263238]">—</span>
                  )}
                </td>
                {/* Meeting Day */}
                <td className="px-5 py-3">
                  {m.meeting_day ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm text-[#263238]">{m.meeting_day}</span>
                    </div>
                  ) : <span className="text-sm text-[#263238]">—</span>}
                </td>
                {/* Meeting Time */}
