...
import { eventsApi, serviceTypesApi, specialEventTypesApi } from '@/lib/api';
...
const EMPTY = {
  ...
  recurrence_pattern: '', days_of_week: [] as string[],
};
...
export default function EventForm({ eventId, eventKind }: Props) {
  ...
  const [form,    setForm]    = useState({ ...EMPTY, event_type: '' });
  const [saving,  setSaving]  = useState(false);
  const [statusActionLoading, setStatusActionLoading] = useState(false);
  const [loading, setLoading] = useState(isEdit);
  const [errors,  setErrors]  = useState<Record<string, string>>({});

  // Fetch active types live from the API based on which kind of event this is
  useEffect(() => {
    setTypesLoading(true);
    const api = eventKind === 'service' ? serviceTypesApi : specialEventTypesApi;
    api.list()
      .then((r: any) => {
        const list = (r.data.results || r.data).filter((t: any) => t.is_active);
        const opts = list.map((t: any) => ({ value: t.key, label: t.name }));
        setTypeOptions(opts);
        // Default to first available type only when adding a new event
        if (!isEdit && opts.length > 0) {
          setForm(f => (f.event_type ? f : { ...f, event_type: opts[0].value }));
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Either a start date OR (recurring + at least one day) is required
    if (!form.start_date && !(form.is_recurring && form.days_of_week.length > 0)) {
      setErrors({ start_date: 'Provide a start date, or mark as recurring and select at least one day.' });
      toast.error('Date or days required', 'Set a start date, or select recurring days of the week.');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        days_of_week:       form.is_recurring ? form.days_of_week : [],
      };
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div>
            ...
            {!typesLoading && typeOptions.length === 0 && (
              <p className="text-xs text-amber-600 mt-1.5">
                No {eventKind === 'service' ? 'Service' : 'Special Event'} Types exist yet.{' '}
                <a
                  href={eventKind === 'service' ? '/events/service-types/add' : '/events/special-event-types/add'}
        ...
        <div className="p-6 grid grid-cols-2 gap-5">
          <div>
            ...
            {getInput('start_date', { type: 'date' })}
            {getError('start_date')}
            <p className="text-xs text-gray-400 mt-1.5">
              Optional for recurring services — select days of the week below instead.
            </p>
          </div>
          <div>
            ...
            <div className="p-6 space-y-4">
              ...
              {form.is_recurring && (
                <div className="space-y-4">
                  ...
                  <div>
                    <FieldLabel label="Days of the Week" />
                    <div className="flex flex-wrap gap-2">
                      {DAYS_OF_WEEK.map(d => {
                        const isOn = form.days_of_week.includes(d.value);
                        return (
                          <button
                            key={d.value}
                            type="button"
                            onClick={() => {
                              setForm(f => ({
                                ...
                                days_of_week: isOn
                                  ...
                                  : [...f.days_of_week, d.value],
                              }));
                              setErrors(fe => ({ ...fe, start_date: '' }));
                            }}
                            className={`w-12 h-12 rounded-xl text-xs font-bold transition-colors border ${
                              isOn
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                      Select every day this service repeats on, e.g. Sunday and Wednesday.
                      {form.days_of_week.length > 0 && !form.start_date && (
                        <span className="text-indigo-500 font-medium"> No specific start date needed.</span>
                      )}
                    </p>
                  </div>
                </div>
