...
import { Plus, Edit2, Trash2, Search, Calendar, Clock, MapPin, RefreshCw, Users } from 'lucide-react';
...
export default function EventListPage({
  eventKind, title, subtitle, addPath, editPath, typeFilter
}: Props) {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      // Filter by event_type relevant to this page
      const list = all.filter((e: any) => typeFilter.includes(e.event_type));
      setEvents(list);
      setFiltered(list);
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <table className="w-full">
          <thead style={{ background: '#f8fafc' }}>
            <tr>
              {['Title', 'Type', 'Date', 'Time', 'Location', 'Capacity', 'Status', 'Actions'].map(h => (
                ...
                <td className="px-5 py-3">
                  {ev.location ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm text-gray-700 truncate max-w-32">{ev.location}</span>
                    </div>
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm text-gray-700">
                      {ev.max_capacity ?? <span className="text-gray-400">Unlimited</span>}
                    </span>
                  </div>
                </td>
                {/* Status */}
