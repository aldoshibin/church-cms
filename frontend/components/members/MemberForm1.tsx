...
export default function MemberForm({ memberId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Send as JSON (profile picture handled separately if needed)
      const payload = {
        ...form,
    ...
    <form onSubmit={handleSubmit}>
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="p-6 flex items-center gap-6">
          ...
          <div>
            <label className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 text-sm font-semibold rounded-lg border border-indigo-200 hover:bg-indigo-100 cursor-pointer transition-colors">
              <Upload size={14} /> Upload Photo
              <input type="file" accept="image/*" onChange={handleProfileChange} className="hidden" />
            </label>
            <p className="text-xs text-gray-400 mt-2">JPG, PNG up to 5MB. Passport size recommended.</p>
          </div>
        </div>
